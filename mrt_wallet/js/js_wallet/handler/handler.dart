...
import '../page/networks/eth.dart';
import '../utils/utils.dart';
import 'completer.dart';
import 'package:mrt_native_support/web/mrt_native_web.dart';

part "js_wallet_handler.dart";
part "networks/ethereum.dart";
part "extention.dart";

abstract class JSWalletAuthenticated {
  abstract Web3APPAuthentication _auth;
  void _sendMessageToClient(WalletResponse response);
}

abstract class WalletHandler extends JSWalletAuthenticated
    with JsEthereumHandler {
  // final _lock = SynchronizedLock();
  String get clientId;
  late final String _id = "WALLET_$clientId";
  ...
  WalletHandler._(this._crypto);

  void _onClientEvent(CustomEvent response) {
    final ClientEthereumRequest request = ClientEthereumRequest.deserialize(
  ...
  void _sendMessageToClient(WalletResponse response) {
    final event = CustomEvent.create(
        type: "ETH_$clientId", data: response.toCbor().encode(), clone: true);
  ...
  Future<void> _buildAndSendMessage(
      {required ClientRequest? params, required String requestId}) async {
    try {
  ...
  Future<Web3MessageCore> _completeMessage(ClientRequest params) async {
    try {
  ...
  Future<WalletRequestResponse> _completeJsRequest(ClientRequest params) async {
  ...
  Future<Web3MessageCore> _createMessage(ClientRequest params) async {
    if (params is ClientEthereumRequest) {
      return await _buildEthereumRequest(params);
    }
    throw Web3RequestExceptionConst.invalidRequest;
  }

  WalletRequestResponse _parseWalletResponse(
      {required Web3MessageCore message, required ClientRequest request}) {
    Object? data;
    WalletRequestResponseType type = WalletRequestResponseType.success;
    switch (message.type) {
      case Web3MessageTypes.response:
      ...
      case Web3MessageTypes.error:
        ...
        type = WalletRequestResponseType.failed;
        break;
      default:
