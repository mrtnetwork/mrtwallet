...
/// add testnet coin
class SetupSolanaAddressView extends StatefulWidget {
  const SetupSolanaAddressView({super.key});

  @override
  State<SetupSolanaAddressView> createState() => _SetupSolanaAddressViewState();
}

class _SetupSolanaAddressViewState extends State<SetupSolanaAddressView>
    with SafeState {
  ...
  APPSolanaNetwork get network => networkAccounts.network as APPSolanaNetwork;
  List<CryptoCoins> get coins => network.coins;
  CryptoCoins get coin => coins.firstWhere((element) =>
      element.conf.type ==
      (selectedCustomKey?.type ?? EllipticCurveTypes.ed25519));
  bool inited = false;
