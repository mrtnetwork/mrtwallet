...
import 'package:mrt_wallet/future/pages/wallet_pages/global_pages/importing_custom_key_view.dart';
import 'package:mrt_wallet/future/pages/wallet_pages/setup_pages/mnemonic_view.dart';
import 'package:mrt_wallet/future/pages/wallet_pages/setup_pages/verify_mnemonic.dart';
  ...
  void _generatePrivateKey() async {
    if (formKey.currentState?.validate() ?? false) {
      ...
      if (result.hasError) {
        ...
        final key = await MethodCaller.call(
            () async => TonUtils.generateTonPrivateKeyFromSeed(
                mnemonic: mnemonicList,
                password: password ?? "",
                validateTonMnemonic: validateTonMnemonic),
            delay: AppGlobalConst.milliseconds100);
        if (key.hasError) {
          progressKey.errorText(result.error!.tr);
          mnemonic = "";
          mnemonicList.clear();
      ...
      child: Column(
        children: [
          ...
          AppTextField(
            ...
            validator: state.mnemonicLengthValidator,
            minlines: 3,
...
//               FixedElevatedButton(
//                 onPressed: () {
//                   context.to(PagePathConst.importAccount, argruments: key);
//                 },
