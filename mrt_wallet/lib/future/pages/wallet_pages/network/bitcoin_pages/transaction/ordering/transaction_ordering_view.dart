...
class _OutputWithKey<T> {
  ...
  factory _OutputWithKey._(
      BitcoinBaseOutput output, AppBitcoinNetwork network) {
    if (output is BitcoinScriptOutput) {
      return _OutputWithKey(
          output: output,
          value: NoneDecimalBalance.zero(network.coinParam.decimal),
          addressView: null,
          script: BitcoinUtils.opReturnToView(output.script));
    }
  ...
  final NoneDecimalBalance value;
  final String? script;
...
class _InputsWithKey {
  ...
  late final inputValue =
      NoneDecimalBalance(item.utxo.value, BitcoinUtils.decimal);
}
