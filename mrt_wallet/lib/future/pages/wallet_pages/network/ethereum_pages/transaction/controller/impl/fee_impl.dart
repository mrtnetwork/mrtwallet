...
mixin ETHTransactionFeeImpl on EthTransactionImpl {
  ...
  bool get eip1559 => network.coinParam.supportEIP1559;
  ...
  void onFeeChanged();
  Future<(BigInt?, FeeHistorical?)?> _fetchGasPrice() async {
    try {
      if (eip1559) {
  ...
  Stream<(BigInt?, FeeHistorical?)> _prediocFetchGasStream() async* {
    while (!deleted) {
      ...
      final gas = await _fetchGasPrice();
      if (gas == null) {
  ...
  Future<void> estimateGasLimit({Map<String, dynamic>? estimateDetails}) async {
    ...
    onFeeChanged();
    final estimate = await MethodCaller.call(() async {
