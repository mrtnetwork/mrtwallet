...
class __MetadataContentState extends State<BCMRUriValidateView> with SafeState {
  ...
  String? _validateIpfs(String? v) {
    final cid = AppStringUtility.validateIpfsCIDV1(v);
    if (cid == null) {
  ...
  String? _validateHttps(String? v) {
    final httpsUri = AppStringUtility.validateUri(v, schame: ["https"]);
    if (httpsUri == null) {
  ...
  void readContent() async {
    ...
    String uri = _uri;
    if (isIpfs) {
      uri = AppStringUtility.toIpfsV1Uri(uri);
    }
    final result = await MethodCaller.call(
        () async => await BitcoinCashUtils.getBCMR(uri));
    if (result.hasError) {
