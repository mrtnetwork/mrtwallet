...
class _CardanoTransactionAssetSelectorViewState
  ...
  Map<_AssetWithPolicyId, NoneDecimalBalance> assetsToTransfer = {};
    ...
    for (final i in widget.receiver.asset.assets.entries) {
      for (final b in i.value.assets.entries) {
        try {
          ...
          assetsToTransfer[a] =
              NoneDecimalBalance(b.value.balance, b.value.currencyDecimal);
        } on StateError {
          continue;
        }
      }
  ...
  void onSelectAsset(
    ...
    try {
      ...
      assetsToTransfer[assets] = NoneDecimalBalance(val, assets.decimal);
    } finally {
      setState(() {});
    }
  }
