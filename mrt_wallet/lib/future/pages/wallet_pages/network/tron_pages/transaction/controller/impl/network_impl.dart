...
mixin NetworkConditionImpl on TronTransactionImpl {
  ...
  Future<bool> initNetworkCondition() async {
    ...
    if (result.hasError) {
      ...
      final initFields = await MethodCaller.call(() async {
  ...
  bool _checkPermission(TronAccountInfo account) {
    ...
    if (address.multiSigAccount) {
      ...
      final List<PermissionKeys> signers =
          msigAccount.multiSignatureAccount.signers
              .map((e) => PermissionKeys(
                  ...
                  weight: e.weight))
              .toList();
      final findPermission = MethodCaller.nullOnException(() =>
      ...
      for (final i in findPermission.keys) {
        final findSigner = MethodCaller.nullOnException(() =>
            signers.firstWhere((element) =>
                element.address.toAddress() == i.address.toAddress()));
    ...
    for (final i in accountAccess) {
      final accKey = MethodCaller.nullOnException(() => i.keys.firstWhere(
