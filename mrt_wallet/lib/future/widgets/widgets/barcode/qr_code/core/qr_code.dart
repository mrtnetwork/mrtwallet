...
class QrCode {
  ...
  factory QrCode.fromBytes({
    required List<int> data,
    required int errorCorrectLevel,
  }) {
    final typeNumber = _calculateTypeNumberFromData(
      errorCorrectLevel,
      [QrByte.fromBytes(data)],
    );
    return QrCode(typeNumber, errorCorrectLevel)
      .._addToList(QrByte.fromBytes(data));
  }
...
List<int> _createBytes(QrBitBuffer buffer, List<QrRsBlock> rsBlocks) {
  ...
  for (var r = 0; r < rsBlocks.length; r++) {
    ...
    final dcItem = dcData[r] = List<int>.filled(dcCount, 0, growable: false);
    ...
    final ecItem =
        ecData[r] = List<int>.filled(rsPoly.length - 1, 0, growable: false);
