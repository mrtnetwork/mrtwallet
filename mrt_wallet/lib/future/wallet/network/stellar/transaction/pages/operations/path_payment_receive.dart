...
import 'package:mrt_wallet/wallet/api/client/networks/networks.dart';
...
class PathPaymentStrictReceiveOperationView extends StatefulWidget {
  final StellarTransactionStateController controller;
  final StellarPathPaymentStrictReceiveOperation? operation;
  const PathPaymentStrictReceiveOperationView(
      {required this.controller, this.operation, Key? key})
      : super(key: key);

  @override
  State<PathPaymentStrictReceiveOperationView> createState() =>
      _PathPaymentStrictReceiveOperationViewState();
}

class _PathPaymentStrictReceiveOperationViewState
    extends State<PathPaymentStrictReceiveOperationView>
  ...
  late IntegerBalance destAmount =
  ...
  void stupDestAmount(BigInt? amount) {
    if (amount == null) return;
    destAmount.updateBalance(amount);
    checkIsReady();
    updateState();
  }
  ...
  void setupOperation() {
    if (!isReady) return;
    final operation = StellarPathPaymentStrictReceiveOperation(
        asset: asset!,
        sendAmount: amount,
  ...
  void init() {
    ...
    destAmount.updateBalance(operation.destAmount.balance);
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        PageTitleSubtitle(
            title: "stellar_path_payment_strict_receive".tr,
            body: LargeTextView([
              "stellar_path_payment_strict_receive_desc".tr,
              if (widget.operation != null) ...[
                "remove_operation_close_page_desc".tr,
              ]
            ])),
        Text("send_asset".tr, style: context.textTheme.titleMedium),
        Text("stellar_path_receive_send_asset_desc".tr),
...
class _Amount extends StatelessWidget {
  final _PathPaymentStrictReceiveOperationViewState state;
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
            ...
            context
                .openSliverBottomSheet<BigInt>(
                  ...
                  child: SetupNetworkAmount(
                    ...
                    subtitleText: "stellar_path_receive_send_max_desc".tr,
                    min: BigInt.zero,
                  ),
                )
                    ...
                    bodyBuilder: (c) =>
                        SelectRecipientAccountView<StellarAddress>(
                          ...
                          subtitle: PageTitleSubtitle(
                              title: "destination".tr,
                              body: Text(
                                  "stellar_path_receive_destination_desc".tr)),
                        ))
                .then(state.setReceiver);
          },
        ),
        ...
        LargeTextView(["stellar_path_receive_dest_asset_desc".tr], maxLine: 2),
        WidgetConstant.height8,
        ContainerWithBorder(
                ...
                children: [
                  ...
                  TransactionAmountView(
                    ...
                    onTap: () {
                      context
                          .openSliverBottomSheet<BigInt>(
                            "destination_amount".tr,
                            child: SetupNetworkAmount(
                              buttonText: "setup_amount".tr,
                              token: state.destAssets!.token,
                              subtitleText:
                                  "stellar_path_receive_dest_amount_desc".tr,
                  ...
                  LargeTextView(["stellar_path_receive_path_desc".tr],
                      maxLine: 2),
                  WidgetConstant.height8,
