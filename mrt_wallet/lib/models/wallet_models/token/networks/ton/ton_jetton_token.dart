...
class TonJettonToken with Equatable implements TokenCore<BigInt> {
  ...
  factory TonJettonToken.fromCborBytesOrObject(
    ...
    try {
      final CborListValue cbor = CborSerializable.decodeCborTags(
          bytes, obj, WalletModelCborTagsConst.jettonToken);
      ...
      final Live<NoneDecimalBalance> balance =
          Live(NoneDecimalBalance(cbor.elementAt(3), token.decimal!));
  ...
  TonJettonToken updateToken(Token updateToken) {
    return TonJettonToken._(
        balance: Live(
            NoneDecimalBalance(balance.value.balance, updateToken.decimal!)),
        token: updateToken,
        minterAddress: minterAddress,
        walletAddress: walletAddress,
        updated: updated,
  ...
  CborTagValue toCbor() {
    return CborTagValue(
        ...
        WalletModelCborTagsConst.jettonToken);
  }
